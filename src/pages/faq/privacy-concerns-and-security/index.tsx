import React from "react";

import { useTranslation } from "next-i18next";

import FaqHeader from "@/components/faq/header";
import Footer from "@/shared/footer";
const Component: React.FC = () => {
  const { t } = useTranslation("faq");
  return (
    <div className="flex min-h-screen flex-col">
      <div className="my-10">
        <FaqHeader />
        <div className="mx-auto mt-10 max-w-3xl">
          <h2 className="mb-4 text-2xl font-bold">{t("privacy")}</h2>
          <div className="rounded bg-white p-4 shadow">
            <h3 className="mb-2 text-xl font-bold">{t("personal_secure")}</h3>
            <p>{t("personal_secure_description")}</p>

            <h3 className="my-2 text-xl font-bold">{t("person_protected")}</h3>
            <p>{t("person_protected_description")}</p>

            <h3 className="my-2 text-xl font-bold">
              {t("chat_history_stored")}
            </h3>
            <p>{t("chat_history_stored_description")}</p>

            <h3 className="my-2 text-xl font-bold">{t("delete_personal")}</h3>
            <p>{t("delete_personal_description")}</p>

            <h3 className="my-2 text-xl font-bold">
              {t("conpliant_regulations")}
            </h3>
            <p>{t("conpliant_regulations_description")}</p>

            <h3 className="my-2 text-xl font-bold">{t("security_measures")}</h3>
            <p>{t("security_measures_description")}</p>
          </div>
        </div>
      </div>
      <Footer />
    </div>
  );
};

Component.displayName = "PrivacyConcernsAndSecurity";
export default Component;
